...
use crate::{robot::Robot, utils::{make_grid, valid_bounded_move}};
...
#[instrument]
pub fn execute_robot(
    robot: &mut Robot,
    planetary_grid: &mut Vec<Vec<char>>,
) {
    ...
    for command in robot.command_queue.iter() {
        ...
        match command {
            ...
            'F' => {
                ...
                let current_cell = planetary_grid[robot.coordinate[1] as usize][robot.coordinate[0] as usize];

                let has_scent = current_cell == 'X';

                if !has_scent {
                    if !valid_move {
                        planetary_grid[robot.coordinate[1] as usize][robot.coordinate[0] as usize] = 'X';
    ...
    if robot.is_simulating {
        let str_to_print = format!("{:?} {:?} {:?} ", robot.coordinate[0], robot.coordinate[1], robot.direction).replace("'", "");
        print!("{}", str_to_print);
    }
}
...
impl Mars {
    ...
    pub fn simulate(&mut self) {
        ...
        for i in 0..range {
            execute_robot(&mut self.robots[i], &mut self.planetary_grid);
        }
    }
}
